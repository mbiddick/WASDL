...
Answers questions ONLY from your uploaded knowledge base — no hallucinations.

Setup:
  pip install -r requirements.txt
  export ANTHROPIC_API_KEY="sk-ant-..."
  export ADMIN_PASSWORD="yourpassword"   # optional, default: debate2024
  export BOT_NAME="Debate Assistant"    # optional
  python app.py
"""
...
try:
    import pdfplumber  # Best for tables — pip install pdfplumber
    HAS_PDFPLUMBER = True
except ImportError:
    HAS_PDFPLUMBER = False

try:
    import fitz  # PyMuPDF fallback — pip install pymupdf
    HAS_PDF = True
except ImportError:
    HAS_PDF = False
...
def extract_pdf(file_bytes: bytes) -> str:
    ...
    Uses pdfplumber (best for tables) with PyMuPDF as fallback.
    Tables are converted to readable row-by-row text so they are searchable.
    """
    if HAS_PDFPLUMBER:
        pages = []
        with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
            for i, page in enumerate(pdf.pages):
                ...
                # Tables — convert each row to readable "Label: Value" style text
                ...
                for table in tables:
                    ...
                    for row_idx, row in enumerate(table):
                        if row_idx == 0 and has_headers:
                            continue  # skip header row, use it as labels below
                        cells = [str(c).strip() if c else "" for c in row]
                        if not any(cells):
                            continue
                        if has_headers:
                            # "Header: Value" pairs make content searchable
                            pairs = [f"{h}: {v}" for h, v in zip(headers, cells) if h or v]
                            table_lines.append(" | ".join(pairs))
                        else:
        ...
        # Fallback to PyMuPDF (text only, no table awareness)
        ...
        raise ValueError("No PDF library installed. Run: pip install pdfplumber")
...
# Debate-specific synonym/abbreviation dictionary
DEBATE_SYNONYMS = {
    # Argument types
    ...
    "perm":             ["permutation"],
    "solvency":         ["solve", "solves", "solvency advocate"],
    "inherency":        ["inherent", "status quo"],
    "framework":        ["fw", "value framework", "criterion"],
    "fw":               ["framework"],
    # Evidence
    "evidence":         ["ev", "card", "cards", "evidence card"],
    "ev":               ["evidence", "card"],
    "card":             ["evidence", "ev"],
    "warrant":          ["warrants", "warranted", "reasoning"],
    # Sides
    "affirmative":      ["aff", "pro"],
    "aff":              ["affirmative", "pro"],
    "negative":         ["neg", "con"],
    "neg":              ["negative", "con"],
    # Formats
    "public forum":     ["pf", "pfd", "public forum debate"],
    "pf":               ["public forum", "public forum debate"],
    "lincoln douglas":  ["ld", "lincoln-douglas", "l-d"],
    "ld":               ["lincoln douglas", "lincoln-douglas"],
    "policy debate":    ["cx", "policy", "cross-ex"],
    ...
    # Flow / procedure
    "cross examination":["cross-ex", "crossfire", "cross ex", "cx"],
    "crossfire":        ["cross examination", "cross-ex"],
    "flow":             ["flowing", "flowed"],
    "rebuttal":         ["rebuttals", "rebutting", "rebut"],
    "contention":       ["contentions", "argument", "point"],
    "speaker points":   ["speaks", "speaker point", "points"],
    "speaks":           ["speaker points", "speaker point"],
    "ballot":           ["ballots", "judge ballot", "decision"],
    "resolution":       ["res", "topic"],
    "res":              ["resolution", "topic"],
    "time":             ["timing", "time limit", "time limits"],
    "prep time":        ["prep", "preparation time"],
    "prep":             ["prep time", "preparation time"],
}

def normalize_text(text: str) -> str:
    """Lowercase and normalize compound words."""
    text = text.lower()
    # Normalize hyphens: counter-plan -> counterplan
...
def expand_query(query: str) -> set[str]:
    """
    Expand query keywords with:
    - Stemmed variants
    - Debate-specific synonyms and abbreviations
    - Compound word normalization (counter plan <-> counterplan)
    """
    expanded: set[str] = set()
    q_lower = query.lower()
    q_norm  = normalize_text(query)

    # Base keywords from original and normalized query
    base = keywords(q_lower) | keywords(q_norm)
    expanded.update(base)

    # Add stemmed versions
    expanded.update(stem_word(w) for w in base)

    # Synonym expansion — single words
    for word in list(base):
        if word in DEBATE_SYNONYMS:
            for syn in DEBATE_SYNONYMS[word]:
                expanded.update(keywords(syn))
                expanded.add(normalize_text(syn).replace(" ", ""))

    # Synonym expansion — multi-word phrases in original query
    for phrase, synonyms in DEBATE_SYNONYMS.items():
        if phrase in q_lower or phrase in q_norm:
            for syn in synonyms:
                expanded.update(keywords(syn))

    # Also try run-together version of query words (counterplan from "counter plan")
    expanded.add(re.sub(r"\s+", "", q_norm))

    return expanded - STOP_WORDS

def search(query: str, chunks: list[dict], top_k: int = 6) -> list[dict]:
    """Enhanced keyword search with synonym expansion, stemming, and normalization."""
    if not chunks:
        return []
    ...
    for chunk in chunks:
        ...
        word_list = re.findall(r"\b[a-z]{2,}\b", norm_text)
        doc_len   = len(word_list) + 1
        c_kw      = {w for w in word_list if w not in STOP_WORDS}
        c_kw_stemmed = {stem_word(w) for w in c_kw}
        c_kw_all  = c_kw | c_kw_stemmed

        score = 0.0
        for kw in q_kw:
            kw_stem = stem_word(kw)
            if kw in c_kw_all or kw_stem in c_kw_all:
                tf = (word_list.count(kw) + word_list.count(kw_stem)) / doc_len
                score += (tf + 0.01) * (1 + 1 / max(len(kw) ** 0.3, 1))

        # Bonus for longer exact matches
        for kw in q_kw:
            if len(kw) > 5 and kw in norm_text:
                score += 0.2

        # Bonus for multi-word synonym phrase matches in chunk
        q_lower = query.lower()
        for phrase in DEBATE_SYNONYMS:
            if len(phrase) > 4 and phrase in q_lower and phrase in norm_text:
                score += 0.4
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .welcome-card .tips { margin-top: 20px; text-align: left; background: var(--light); border-radius: 8px; padding: 14px 16px; }
  .welcome-card .tips p { font-size: 13px; margin-bottom: 4px; }
  .welcome-card .tips p:last-child { margin-bottom: 0; }
  ...
  .bubble { padding: 12px 16px; border-radius: var(--radius); font-size: 14px; line-height: 1.6; max-width: 100%; word-break: break-word; }
  .message.user .bubble { background: var(--navy); color: var(--white); border-top-right-radius: 4px; }
  .message.bot  .bubble { background: var(--white); border: 1px solid var(--border); border-top-left-radius: 4px; box-shadow: 0 1px 6px rgba(0,0,0,.06); }
  ...
  .source-chip { background: #eff6ff; color: #1d4ed8; border: 1px solid #bfdbfe; border-radius: 20px; padding: 2px 10px; font-size: 11px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 180px; }
  .typing-bubble { background: var(--white); border: 1px solid var(--border); border-radius: var(--radius); border-top-left-radius: 4px; padding: 14px 18px; display: flex; gap: 5px; align-items: center; box-shadow: 0 1px 6px rgba(0,0,0,.06); }
  ...
  .input-wrap { display: flex; gap: 10px; align-items: flex-end; background: var(--light); border: 1.5px solid var(--border); border-radius: 12px; padding: 10px 14px; transition: border-color .2s; }
  .input-wrap:focus-within { border-color: var(--navy2); background: var(--white); }
  #user-input { flex: 1; border: none; background: transparent; font-size: 14px; line-height: 1.5; resize: none; outline: none; max-height: 120px; min-height: 24px; font-family: inherit; }
  .send-btn { background: var(--navy); color: var(--white); border: none; border-radius: 8px; width: 36px; height: 36px; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: background .2s; font-size: 16px; }
  ...
  .no-kb-banner { background: #fffbeb; border: 1px solid #fde68a; border-radius: 8px; padding: 10px 16px; font-size: 13px; color: #92400e; margin: 0 20px 0; display: flex; align-items: center; gap: 8px; }
  .no-kb-banner.hidden { display: none; }
  .chat-messages::-webkit-scrollbar, .admin-body::-webkit-scrollbar { width: 5px; }
  .chat-messages::-webkit-scrollbar-track, .admin-body::-webkit-scrollbar-track { background: transparent; }
  .chat-messages::-webkit-scrollbar-thumb, .admin-body::-webkit-scrollbar-thumb { background: var(--border); border-radius: 10px; }
  @media (max-width: 640px) {
    .admin-panel { width: 100%; min-width: unset; position: absolute; z-index: 50; height: 100%; top: 64px; left: 0; }
...
<header>
  <div class="header-left">
    ...
    <div class="brand">
      ...
      <p>AI Knowledge Base · Coaches &amp; Judges</p>
    </div>
  </div>
...
<div class="main">
      ...
      <div id="lock-screen">
        ...
        <div class="lock-screen">
          ...
          <div>
            ...
            <input type="password" id="admin-pw" placeholder="Enter password…" onkeydown="if(event.key==='Enter')verifyAdmin()" />
          </div>
          <button class="btn btn-primary btn-block" onclick="verifyAdmin()">Unlock →</button>
          <div class="alert alert-error" id="pw-error">Incorrect password. Try again.</div>
        </div>
      </div>
      <div id="admin-content" style="display:none; display:flex; flex-direction:column; gap:20px;">
        <div>
          <div class="admin-section-title">Upload Documents</div>
          <div class="upload-zone" id="upload-zone"
               ondragover="event.preventDefault();this.classList.add('drag')"
               ondragleave="this.classList.remove('drag')"
               ondrop="handleDrop(event)">
            <input type="file" id="file-input" multiple accept=".pdf,.docx,.txt" onchange="handleFileSelect(event)" />
        ...
        <div>
          <div class="admin-section-title">Knowledge Base (<span id="doc-count">0</span> docs)</div>
  ...
  <section class="chat-area">
    <div class="no-kb-banner hidden" id="no-kb-banner">
      ⚠️ No documents in the knowledge base yet. Open Admin → upload files so the bot can answer questions accurately.
    </div>
    <div class="chat-messages" id="chat-messages">
      <div class="welcome-card" id="welcome-card">
        <div class="big-icon">⚖️</div>
        <h2 id="bot-name-welcome">Debate Assistant</h2>
        <p>Your AI-powered resource for high school debate rules, procedures, and best practices. Ask me anything — I'll answer only from your uploaded knowledge base.</p>
        <div class="tips">
          <p><span class="tip-label">💡 Try asking:</span></p>
          <p>• "What are the speaker point guidelines?"</p>
          <p>• "How does time allocation work in Policy debate?"</p>
          <p>• "What is a topicality argument?"</p>
          <p>• "Can the Aff run a counterplan?"</p>
        </div>
      </div>
    </div>
    <div class="input-bar">
      <div class="input-wrap">
        <textarea id="user-input" rows="1" placeholder="Ask a debate question…" onkeydown="handleKey(event)" oninput="autoResize(this)"></textarea>
        <button class="send-btn" id="send-btn" onclick="sendMessage()">➤</button>
      </div>
      <div class="input-hint">Answers come only from your uploaded documents · Press Enter to send</div>
    </div>
  </section>
</div>
<script>
...
async function init() {
  try {
    ...
    if (s.has_api_key) { dot.className = 'status-dot online'; text.textContent = 'Online'; }
    else { dot.className = 'status-dot offline'; text.textContent = 'No API key'; }
    if (s.document_count === 0) document.getElementById('no-kb-banner').classList.remove('hidden');
    updateDocCount(s.document_count);
...
async function verifyAdmin() {
  ...
  try {
    const r = await fetch('/api/verify-admin', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({password:pw}) });
...
async function loadDocuments() {
  try {
    ...
    renderDocList(docs); updateDocCount(docs.length);
    const banner = document.getElementById('no-kb-banner');
    if (docs.length === 0) banner.classList.remove('hidden');
...
function renderDocList(docs) {
  ...
  el.innerHTML = docs.map(d => {
    ...
    return `<div class="doc-item"><div class="doc-left"><span class="doc-icon">${icon}</span><div class="doc-info"><div class="doc-name" title="${d.name}">${d.name}</div><div class="doc-meta">${d.chunk_count} chunks · ${date}</div></div></div><button class="btn btn-danger btn-sm" onclick="deleteDoc('${d.id}','${d.name}')">✕</button></div>`;
  }).join('');
}
...
async function deleteDoc(id, name) {
  ...
  try {
    const r = await fetch(`/api/documents/${id}`, { method:'DELETE', headers:{'Content-Type':'application/json'}, body:JSON.stringify({password:adminPW}) });
    const d = await r.json();
    if (d.success) loadDocuments();
  } catch(e) { alert('Delete failed. Please try again.'); }
}

function updateDocCount(n) { document.getElementById('doc-count').textContent = n; }
function handleFileSelect(e) { uploadFiles(e.target.files); }
function handleDrop(e) { e.preventDefault(); document.getElementById('upload-zone').classList.remove('drag'); uploadFiles(e.dataTransfer.files); }
...
async function uploadFile(file) {
  ...
  if (!allowed.includes(ext)) { showAlert('upload-error', `"${file.name}" is not a supported type (PDF, DOCX, TXT).`); return; }
  ...
  try {
    ...
    if (d.success) showAlert('upload-success', `✓ "${file.name}" uploaded (${d.chunks_created} chunks created)`);
    else showAlert('upload-error', d.error || 'Upload failed.');
  } catch(e) { bar.style.display='none'; showAlert('upload-error','Upload failed. Check your connection.'); }
}
...
async function sendMessage() {
  ...
  try {
    ...
    if (d.error) appendMessage('bot', '⚠️ ' + d.error, []);
    else { messages.push({role:'assistant', content:d.response}); appendMessage('bot', d.response, d.sources||[]); }
  } catch(e) { removeTyping(typingId); appendMessage('bot','⚠️ Could not reach the server. Please try again.',[]);}
...
function appendMessage(role, text, sources) {
  ...
  if (sources && sources.length) {
    ...
    srcDiv.className = 'sources-tag';
    srcDiv.innerHTML = `<span class="source-label">Sources:</span>` + uniqueSrc.map(s=>`<span class="source-chip" title="${s}">${s}</span>`).join('');
...
function appendTyping(id) {
  ...
  div.innerHTML = `<div class="avatar">⚖️</div><div class="typing-bubble"><div class="dot"></div><div class="dot"></div><div class="dot"></div></div>`;
  container.appendChild(div);
  container.scrollTop = container.scrollHeight;
}
...
def upload():
    ...
    doc = {"id": doc_id, "name": filename, "uploaded_at": datetime.now().isoformat(),
           "chunk_count": len(chunks), "size": len(text)}
    chunk_records = [{"id": str(uuid.uuid4()), "doc_id": doc_id, "doc_name": filename,
                      "text": chunk, "index": i} for i, chunk in enumerate(chunks)]
    knowledge_base["documents"].append(doc)
    knowledge_base["chunks"].extend(chunk_records)
...
def chat():
    ...
    system_prompt = f"""You are {BOT_NAME}, an expert AI assistant for high school debate coaches and judges.

CRITICAL RULES — you MUST follow all of these without exception:
1. Answer ONLY using information from the KNOWLEDGE BASE CONTEXT provided below.
2. If the answer is not present in the context, respond exactly with:
   "I don't have that information in my knowledge base. Please consult your coach or the official rulebook."
3. NEVER invent, fabricate, or infer facts, rules, procedures, or examples that aren't explicitly stated in the context.
4. When you answer, mention which source document the information comes from.
5. Be concise, precise, and use clear language suitable for coaches and judges.
6. If asked about something unrelated to debate, politely redirect the user.
...
{ctx_note}"""
    try:
        ...
        return jsonify({"response": response.content[0].text, "sources": [c["doc_name"] for c in relevant]})
    except Exception as e:
...
def chunk_text(text: str, chunk_size: int = 500, overlap: int = 60) -> list[str]:
    ...
    Tries to respect paragraph boundaries first.
    """
    text = re.sub(r"\n{3,}", "\n\n", text)
    text = re.sub(r" {2,}", " ", text)
...
def search(query: str, chunks: list[dict], top_k: int = 5) -> list[dict]:
    """BM25-inspired keyword search over document chunks."""
    if not chunks:
        return []
    q_kw = keywords(query)
    if not q_kw:
    ...
    for chunk in chunks:
        text = chunk["text"]
        word_list = re.findall(r"\b[a-z]{3,}\b", text.lower())
        doc_len = len(word_list) + 1
        c_kw = set(word_list) - STOP_WORDS

        score = 0.0
        for kw in q_kw:
            if kw in c_kw:
                tf = word_list.count(kw) / doc_len
                score += tf * (1 + 1 / max(len(kw) ** 0.3, 1))
        # Bonus for longer keyword matches
        for kw in q_kw:
            if len(kw) > 5 and kw in text.lower():
                score += 0.15
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .admin-body::-webkit-scrollbar-thumb { background: var(--border); border-radius: 10px; }
    ...
    <div class="chat-messages" id="chat-messages">
      ...
      <div class="welcome-card" id="welcome-card">
        ...
        <div class="tips">
          ...
          <p>• "How does time allocation work in Policy debate?"</p>
          <p>• "What is a topicality argument?"</p>
          <p>• "Explain the flow of a Lincoln-Douglas round."</p>
        </div>
      </div>
...
async function init() {
  try {
    ...
    if (s.document_count === 0) {
      document.getElementById('no-kb-banner').classList.remove('hidden');
    }
    updateDocCount(s.document_count);
  } catch(e) {
...
function renderDocList(docs) {
  const el = document.getElementById('doc-list');
  if (!docs.length) {
    el.innerHTML = '<p class="no-docs">No documents uploaded yet.</p>';
    return;
  }
  el.innerHTML = docs.map(d => {
    const ext  = d.name.split('.').pop().toLowerCase();
      ...
      <div class="doc-item">
        <div class="doc-left">
          ...
          <div class="doc-info">
            ...
            <div class="doc-meta">${d.chunk_count} chunks · ${date}</div>
          </div>
        </div>
        <button class="btn btn-danger btn-sm" onclick="deleteDoc('${d.id}','${d.name}')">✕</button>
      </div>`;
  }).join('');
}
...
function updateDocCount(n) {
  document.getElementById('doc-count').textContent = n;
}
...
async function uploadFile(file) {
  ...
  if (!allowed.includes(ext)) {
    showAlert('upload-error', `"${file.name}" is not a supported type (PDF, DOCX, TXT).`);
    return;
  }
  ...
  try {
    ...
    if (d.success) {
      showAlert('upload-success', `✓ "${file.name}" uploaded (${d.chunks_created} chunks created)`);
    } else {
      showAlert('upload-error', d.error || 'Upload failed.');
    }
  } catch(e) {
    bar.style.display = 'none';
    showAlert('upload-error', 'Upload failed. Check your connection.');
  }
}
...
async function sendMessage() {
  ...
  try {
    ...
    if (d.error) {
      ...
      appendMessage('bot', d.response, d.sources || []);
    }
  } catch(e) {
    removeTyping(typingId);
...
function appendMessage(role, text, sources) {
  ...
  if (sources && sources.length) {
    ...
    srcDiv.innerHTML = `<span class="source-label">Sources:</span>` +
      uniqueSrc.map(s => `<span class="source-chip" title="${s}">${s}</span>`).join('');
    bubble.appendChild(srcDiv);
  }
...
def delete_document(doc_id):
    ...
    knowledge_base["documents"] = [
        d for d in knowledge_base["documents"] if d["id"] != doc_id
    ]
    knowledge_base["chunks"] = [
        c for c in knowledge_base["chunks"] if c["doc_id"] != doc_id
    ]
    save_kb(knowledge_base)
    return jsonify({"success": True})
...
def chat():
    ...
    relevant = search(user_msg, knowledge_base.get("chunks", []), top_k=5)
...
if __name__ == "__main__":
    ...
    print(f"{'='*50}")
    print(f"  URL:       http://localhost:{port}")
    print(f"  API key:   {'✓ Configured' if ANTHROPIC_API_KEY else '✗ NOT SET — export ANTHROPIC_API_KEY=...'}")
    print(f"  Admin pw:  {ADMIN_PASSWORD}")
